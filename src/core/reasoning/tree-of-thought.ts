...
export class TreeOfThoughtReasoner {
  ...
  private scoreThought(thought: string, context: Record<string, unknown>): number {
    // Simplified scoring - would use LLM in production
    let score = 0.5;
    if (thought.length > 10) score += 0.1;
    if (context.relevance) score += 0.2;
    return Math.min(1, Math.max(0, score));
  }

  private findBestPath(tree: Map<string, ThoughtNode>, rootId: string): string[] {
