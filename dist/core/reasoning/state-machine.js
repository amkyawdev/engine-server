...
export class StateMachineReasoner {
    ...
    async reason(input, initialContext = {}) {
        ...
        while (context.currentState !== 'finished' && context.currentState !== 'error') {
            ...
            const transition = this.transitions.find((t) => (t.from === context.history[context.history.length - 2] || t.from === '*') &&
                t.to === nextState);
    ...
    findNextState(context) {
        for (const transition of this.transitions) {
            if ((transition.from === context.currentState || transition.from === '*') &&
